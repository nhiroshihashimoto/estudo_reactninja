...
it('every([], (item) => item) shoud return true', () => {
    expect(every([], (item) => item)).to.be.ok
})

it('every([0, 2, 3], (item) => item) shoud return false', () => {
    expect(every([0, 2, 3], (item) => item)).to.be.not.ok
})

it('every([1, 2, 3], (item) => item) shoud return true', () => {
    expect(every([1, 2, 3], (item) => item)).to.be.ok
})

it('every([1, 2, 3], (item, index) => index === item - 1) shoud return true', () => {
    expect(every([1, 2, 3], (item, index) => index === item - 1)).to.be.ok
})

it('every([1, 2, 3], (item, index, array) => array.length === 3) shoud return true', () => {
